...
export default function Home() {
    ...
    const [editingTarget, setEditingTarget] = useState<null | { type: "list" | "todo", id: number }>(null);
    ...
    return (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 place-items-center">
            ...
            {[...lists].reverse().map(l => <ToDoList key={l.id} id={l.id} name={l.name} color={l.color} todos={l.todos} editingTarget={editingTarget} setEditingTarget={setEditingTarget} dispatch={dispatch} />)}
        </div>
    );
}